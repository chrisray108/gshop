...
`OPTION_VALID`  varchar(1) NOT NULL DEFAULT '1',
`OPTION_ORDER`  int NOT NULL auto_increment unique key,
PRIMARY KEY (`OPTION_ID`) 
);

CREATE TABLE `MU_SKU_SPEC_RELATION` (
`RELATION_ID` varchar(64) NOT NULL,
`KEEP_ID` varchar(64) NOT NULL,
`SPEC_OPTION_ID` varchar(64) NOT NULL,
`SPEC_DESC`   varchar(64) NOT NULL,
`SPEC_VALID`  varchar(1) NOT NULL DEFAULT '1',
...
ALTER TABLE `MU_SKU_SPEC_RELATION` ADD CONSTRAINT `fk_ss_relation_keep` FOREIGN KEY (`KEEP_ID`) REFERENCES `MU_SKU` (`KEEP_ID`) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE `MU_SKU_SPEC_RELATION` ADD CONSTRAINT `fk_ss_relation_spec_opt` FOREIGN KEY (`SPEC_OPTION_ID`) REFERENCES `MU_SPEC_OPTION` (`OPTION_ID`) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE `MU_MANAGERS_AUTHORITY` ADD CONSTRAINT `fk_auth_managers` FOREIGN KEY (`MANAGER_ID`) REFERENCES `MU_MANAGERS` (`MANAGER_ID`) ON DELETE CASCADE ON UPDATE CASCADE;
